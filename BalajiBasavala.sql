...
-- 1Q:*Find the most senior employee based on job title 
...
select billing_country,count(*) as countInvoices
from Invoice 
group by billing_country
order by countInvoices desc;
...
from (
        ...
        join invoice i
            on c.customer_id = i.customer_id
        group by c.customer_id, c.first_name, c.last_name, i.billing_country) as t
where t.rank_in_country = 1
order by t.billing_country;
...
with artist_earnings as (
    ...
    group by il.invoice_id, a.artist_id)
...
select
    ...
    ar.artist_id,
    ar.name as artist_name,
    SUM(ae.artist_revenue) as total_spent_on_artist
from customer c
join invoice i 
    on c.customer_id = i.customer_id
join artist_earnings ae 
    on i.invoice_id = ae.invoice_id
join artist ar 
    on ae.artist_id = ar.artist_id
group by
    c.customer_id,
    c.first_name,
    c.last_name,
